...
class UserHandler {
    ...
    async generateASP(user, data) {
        ...
        try {
            await this.users.collection('asps').insertOne(passwordData);
        } catch (err) {
            err.code = 'InternalDatabaseError';
            throw err;
        }
    ...
    async create(data) {
        ...
        if (!data.password) {
            ...
            try {
                if (data.hashedPassword) {
                    ...
                    await hashes.compare('whatever', data.password);
        ...
        let user;
        try {
            let r = await this.users.collection('users').insertOne(userData, { w: 'majority' });
            if (r.insertedId) {
        ...
        if (!data.emptyAddress) {
            ...
            try {
                ...
                await this.users.collection('addresses').insertOne(addressData, { w: 'majority' });
            } catch (err) {
                try {
                    // try to rollback
                    await this.users.collection('users').deleteOne({ _id: user });
    ...
    async updateTeam(team, data) {
        ...
        let result;
        try {
            result = await this.users.collection('users').updateMany(
                { tags: "team:" + team },
                updateQuery,
                {
                    returnOriginal: false,
            ...
            log.error('DB', 'UPDATEFAIL team=%s error=%s', team, err.message);
            err.message = 'Database Error, failed to update team';
            err.code = 'InternalDatabaseError';
            throw err;
        }

        if (!result || !result.value) {
            log.error('DB', 'UPDATEFAIL team=%s error=%s', team, 'Team was not found');
            let err = new Error('team was not found');
            err.code = 'TeamNotFound';
            throw err;
        }

        //FIXME: implement proper team wide logout
        // if (wasSuspended) {
        //     try {
        //         await this.logoutTeam(team, 'Authentication required');
        //     } catch (err) {
        //         // ignore
        //     }
        // }
